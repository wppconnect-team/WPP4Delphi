...
Keep `version` in [boss.json](boss.json) in sync with `TWPPConnectVersion` whenever the latter is bumped — Boss (the Delphi package manager, see `## Build / install`) reads it as the package's published version, and it drifting from the real component version is exactly the kind of staleness that misleads `boss install` users.
