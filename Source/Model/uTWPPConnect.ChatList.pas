...
uses
  {$IFDEF FPC}
  Generics.Collections, uTWPPConnect.Classes, uTWPPConnect.Constant
  {$ELSE}
  Generics.Collections, Rest.Json, uTWPPConnect.Classes, uTWPPConnect.Constant
  {$ENDIF};
...
public
 ...
 // property hydratedButtons: TArray<THydratedButtonsClass> read FHydratedButtons write FHydratedButtons;
  ...
  {$IFDEF FPC}
  //No FPC, instanciar TClassPadraoList<T> a partir de outra unit (nao a que declara
  //o generic) causa um erro de resolucao interno do compilador ("Forward declaration
  //not solved"). TGetChatList ja sombreia FResult/result com campos proprios e nunca
  //usa o array herdado do generic, entao aqui herda direto de TClassPadrao - mesmo
  //comportamento observavel, sem o bug do FPC.
  TGetChatList = class(TClassPadrao)
  {$ELSE}
